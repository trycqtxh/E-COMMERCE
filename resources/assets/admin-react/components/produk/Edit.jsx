...
class Edit extends Component{
	...
	getData(id){
		...
		return new Promise((resolve) => {
			setTimeout(function() {
				util.http.get(`${util.base_url}/produk/${self.state.id}`)
					.then(res => {
						resolve(res.data)
					})
			}, 500);
	...
	simpan(event){
		...
		let self = this.state
		util.http.put(`${util.base_url}/produk/${self.id}`, self.data).then(res => {
				...
				<div className="widget-content">            
					<form className="form-horizontal" ref="Form" onSubmit={this.simpan}>
						<div className="form-group">
							<label className="control-label col-md-2" htmlFor="produk">produk</label>
